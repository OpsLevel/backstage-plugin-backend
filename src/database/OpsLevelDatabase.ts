...
export class OpsLevelDatabase {
  ...
  async fetchConfigValue(key: string): Promise<string | null> {
    const ret = await this.database<OpslevelConfigRow>("opslevel_config")
      .where("key", key)
      .select("value");
    if (ret.length === 0) {
      return null;
  ...
  async setConfigValues(entries: Array<{ key: string; value: string | null }>) {
    ...
    try {
      for (let i = 0; i < entries.length; i++) {
        this.logger.info(
          `Received new OpsLevel configuration record: ${entries[i].key}=${entries[i].value}`,
        );
        await this.database<OpslevelConfigRow>("opslevel_config")
          .transacting(tx)
          .insert(entries[i])
          .onConflict("key")
          .merge();
  ...
  async fetchExportRuns(pageNumber: number | null, pageSize: number | null) {
    ...
    const count: { [key: string]: number | string } | undefined =
      await this.database("opslevel_export_run").count("id as count").first();
    return {
      total_count:
        count === undefined ? 0 : parseInt(count.count.toString(), 10),
      rows: await query.select("*").orderBy("id", "desc"),
    };
  }

  async upsertExportRun(run: OpslevelExportRun): Promise<number> {
    return this.database.transaction().then(async (tx) => {
