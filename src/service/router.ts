...
export async function createRouter(
  ...
  const validatePermissions = (
    _identity: BackstageIdentityResponse | undefined,
  ) => {
    logger.info("Unimplemented access permission check");
  };

  router.get("/ping", async (_, response) => {
    response.json({ status: "OpsLevel Backend Plugin is present" });
  });

  router.get("/auto_sync", async (request, response) => {
    validatePermissions(await identity.getIdentity({ request }));
  ...
  router.post("/auto_sync", async (request, response) => {
    ...
    if (
      ...
      typeof auto_sync_schedule === "string"
    ) {
      // This is what Backstage uses internally:
      try {
        new CronTime(auto_sync_schedule);
      } catch (e) {
        throw new Error("Invalid cron expression");
      }
      if (!auto_sync_schedule.startsWith("0 ")) {
        throw new Error("Can run at most once per hour");
      }
  ...
  router.post("/auto_sync/runs", async (request, response) => {
    ...
    if (
      ...
      return response.json(
        await controller.getAutoSyncExecutions(b.page_number, b.page_size),
      );
    }
    throw new Error("Invalid request format");
  });
