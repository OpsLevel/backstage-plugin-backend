...
export class OpsLevelGraphqlAPI {
  ...
  constructor(backendUrl: string, client: GraphQLClient | null = null) {
    this.client =
      client ||
      new GraphQLClient(`${backendUrl}/api/proxy/opslevel/graphql`, {
        headers: { "GraphQL-Visibility": "internal" },
      });
    this.limiter = new RateLimiter({
      tokensPerInterval: MAX_RPM,
      interval: "minute",
    });
  }
  ...
  public async exportEntity(entity: Entity): Promise<ExportEntityResponse> {
    ...
    return this.request<ExportEntityResponse>(IMPORT_ENTITY_QUERY, {
      entityRef,
      entity,
    });
  }
