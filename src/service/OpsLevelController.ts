...
class RunRecordHandler {
  ...
  static build(db: OpsLevelDatabase) {
    return this.prepareRunRecord(db).then((runRecord) => {
  ...
  public async setState(state: "running" | "completed" | "failed") {
    if (state === "completed") {
      this.runRecord.completed_at = new Date();
    }
  ...
  public async finalize() {
    await this.message(
      `Export to OpsLevel ended with status "${this.runRecord.state}"`,
    );
    await this.updateRunRecord();
  }

  private static async prepareRunRecord(
    db: OpsLevelDatabase,
  ): Promise<OpslevelExportRun> {
...
export class OpsLevelController {
  ...
  public constructor(
    ...
    catalog: CatalogApi,
    config: Config,
    opsLevelApi: OpsLevelGraphqlAPI | null = null,
  ) {
    this.db = db;
    this.logger = logger;
    ...
    this.opsLevel =
      opsLevelApi ||
      new OpsLevelGraphqlAPI(config.getString("backend.baseUrl"));
  }

  public async getAutoSyncConfiguration() {
    return {
      auto_sync_enabled:
        (await this.db.fetchConfigValue("auto_sync_enabled")) === "true",
      auto_sync_schedule:
        (await this.db.fetchConfigValue("auto_sync_schedule")) || "0 * * * *",
    };
  }
  ...
  public async getAutoSyncExecutions(
  ...
  public async scheduleAutoSyncIfApplicable() {
    ...
    const { auto_sync_enabled, auto_sync_schedule } =
      await this.getAutoSyncConfiguration();
    if (auto_sync_enabled) {
      this.logger.info(
        `Scheduling new task according to configuration: ${auto_sync_schedule}`,
      );
      ...
      this.scheduler.scheduleTask({
        ...
        fn: async (abortSignal: AbortSignal) => {
          await this.exportToOpsLevel(abortSignal);
        },
  ...
  public async exportToOpsLevel(abortSignal: AbortSignal) {
    ...
    const recordHandler: RunRecordHandler = await RunRecordHandler.build(
      this.db,
    );
    try {
      for (const kind of ["user", "group", "component"]) {
        const entities = await this.loadEntities(kind, recordHandler);
        await this.performEntityExport(
          entities.items,
          recordHandler,
          abortSignal,
        );
  ...
  private async loadEntities(kind: string, recordHandler: RunRecordHandler) {
    ...
    await recordHandler.message(
      `Loaded ${entities.items.length} entities of type ${kind} from Backstage`,
    );
    return entities;
  }

  private async performEntityExport(
    entities: Array<Entity>,
    recordHandler: RunRecordHandler,
    abortSignal: AbortSignal,
  ) {
    for (const entity of entities) {
      await recordHandler.message(`Exporting ${stringifyEntityRef(entity)}...`);
      try {
        const response: ExportEntityResponse =
          await this.opsLevel.exportEntity(entity);
        await recordHandler.message(
          `Response: ${response.importEntityFromBackstage.actionMessage}`,
        );
        if (response.importEntityFromBackstage.errors.length) {
          await recordHandler.message(
            `Error: ${response.importEntityFromBackstage.errors[0].message}`,
          );
        }
      } catch (e) {
        await recordHandler.message(
