...
describe('OpsLevelGraphqlAPI', () => {
  ...
  describe('exportEntity', () => {
    ...
    it('retries thrice if we are getting rate limited, then gives up', async () => {
      ...
      client.request.mockImplementation(() => { throw error; } );
      jest.spyOn(global, 'setTimeout').mockImplementation((callback) => { callback(); return null as any; } );
      ...
      for(let i = 1; i <= 4; i++) {
        expect(client.request).toHaveBeenNthCalledWith(i, IMPORT_ENTITY_QUERY, { entity: entity, entityRef: "user:default/user 1" })
      }
    });
    ...
    it('retries thrice if we are getting rate limited, then succeeds on the last try', async () => {
      ...
      const error = new ClientError({ status: 429 }, { query: "" });
      let n = 3;
      client.request.mockImplementation(() => { if(n-- > 0) throw error; });
      jest.spyOn(global, 'setTimeout').mockImplementation((callback) => { callback(); return null as any; } );
